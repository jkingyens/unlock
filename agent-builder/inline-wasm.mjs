...
async function inlineWasm() {
    const distDir = 'dist';
    const jsFile = path.join(distDir, 'agent.bundled.js');

    console.log(`Inlining WASM into ${jsFile}...`);
    ...
    for (const wasmFile of wasmFiles) {
        ...
        jsContent = jsContent.split(`'./${wasmFile}'`).join(`'${dataUri}'`);
        jsContent = jsContent.split(`"./${wasmFile}"`).join(`"${dataUri}"`);
        jsContent = jsContent.split(`'${wasmFile}'`).join(`'${dataUri}'`);
        jsContent = jsContent.split(`"${wasmFile}"`).join(`"${dataUri}"`);
    }
