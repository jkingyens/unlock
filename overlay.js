...
async function handleAudioControl(request) {
    ...
    switch (command) {
        case 'play':
            // --- START OF THE FIX ---
            // This logic is now simplified. We always ensure the currentTime is set before playing.
            ...
            // Always set the current time from the provided start time.
            if (data.startTime) {
                audio.currentTime = data.startTime;
            }
            
            // Play the audio. If the source was new, it will play once loaded.
            // If the source was the same, it will resume from the new currentTime.
...
function handleMessages(request, sender, sendResponse) {
  ...
  switch (request.type) {
    case 'control-audio':
        const response = handleAudioControl(request.data);
